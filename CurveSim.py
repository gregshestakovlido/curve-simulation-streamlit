...
class pool:
    ...
    def __init__(self, A, D, n, p=None, tokens=None, fee=4 * 10**6, feemul=None, r=None):
        ...
        if isinstance(n, list):  # is metapool
            ...
            self.tokens = self.D()
            self.feemul = feemul
            self.st_eth_reserves=self.x()*10**18
            self.eth_reserves=self.p()*10**18
            ...
            self.n_total = self.n
            """ self.st_eth_reserves = self.x * 10 ** 18
            self.eth_reserves = self.p * 10 ** 18 #add rates as a object parameters and pass them to session key
...
def pooldata(poolname,balanced=False):
    link=st.secrets['infura_key']
    ...
    pools = pd.DataFrame({'address': '0xDC24316b9AE028F1497c275EB9192a3Ea0f67022', 'coins': '["ethereum", "staked-ether"]',
                         'precmul': '[1,1]', 'basepool': 'None', 'tokentype': 'False', 'feemul': 'None'},
                        index=['steth'])
    pools['coins']=pools['coins'].apply(literal_eval)
    pools['precmul']=pools['precmul'].apply(literal_eval)
    pools['tokentype']=pools['tokentype'].apply(literal_eval)
...
def tokenrate(tokentype, address):
    w3 = Web3(
        Web3.HTTPProvider(
            "https://mainnet.infura.io/v3/0708e00abf26417ebe4aae7eac2d555e"
        )
    )
...
def redemptionprices(n=100):
    url = "https://mainnet.infura.io/v3/0708e00abf26417ebe4aae7eac2d555e"
    query = (
        """
{
...
def find_exchange_rate(type,desired_rate):
    '''
    0-ETH->stETH
    1-stETH->ETH
    '''
